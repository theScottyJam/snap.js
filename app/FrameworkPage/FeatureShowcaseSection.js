...
export const FeatureShowcaseSection = defineStyledElement('FeatureShowcaseSection', getStyles, () => {
  ...
  return html`
    ${renderCodeSamples({ signalSelectedFeature })}
    ${renderFeatureList({ signalSelectedFeature })}
  `;
});

function renderCodeSamples({ signalSelectedFeature }) {
  return html`
    <div class="code-sample-areas-container">
      ${renderEach(
        new Signal(features.map((feature, i) => [i, feature])),
        (feature, i) => renderCodeSample({ feature, index: i, signalSelectedFeature }),
      )}
    </div>
  `;
}

function renderCodeSample({ feature, index, signalSelectedFeature }) {
...
function renderFeatureList({ signalSelectedFeature }) {
  return html`
...
function renderFeatureItem({ featureIndex, signalSelectedFeature, header }) {
  const signalContainerClass = signalSelectedFeature.use(selectedFeature => {
    return 'feature' + (selectedFeature === featureIndex ? ' selected' : '');
  });

  return html`
    <button ${set({
      className: signalContainerClass,
      onmouseover: () => signalSelectedFeature.set(featureIndex),
      onclick: () => signalSelectedFeature.set(featureIndex),
    })}>
