...
If you wish to filter out multiple values, look at [the plain JavaScript version of `_.difference()`](#!/nolodash/difference). Both `_.difference()` and `_.without()` achieve the same effect, but with slightly different function signatures.
